...
fun Modifier.focusHighlight(
    ...
    val finalBorderColor = if (focused && showBorder) (customBorderColor ?: MaterialTheme.colorScheme.primary) else Color.Transparent
    return this
        .onFocusChanged { focused = it.isFocused || it.hasFocus }
        .zIndex(if (focused) 1f else 0f)
        .scale(scale)
        .clip(shape)
        .border(width = 1.dp, color = finalBorderColor, shape = shape)
}
