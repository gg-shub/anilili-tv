...
fun EmbedWebView(
    ...
    var seekTextVisible by remember { mutableStateOf(false) }
    var seekText by remember { mutableStateOf("") }
    LaunchedEffect(seekTextVisible, seekText) {
        if (seekTextVisible) {
            delay(1500)
            seekTextVisible = false
        }
    }
    ...
    androidx.activity.compose.BackHandler(enabled = tvControlsVisible && device.isTv) {
        tvControlsVisible = false
        webView?.requestFocus()
    }
    ...
    val remoteModifier = if (device.isTv && focusPlayerOnStart) {
        Modifier
            .onPreviewKeyEvent { event ->
                if (event.key == androidx.compose.ui.input.key.Key.DirectionLeft || event.key == androidx.compose.ui.input.key.Key.DirectionRight) {
                    if (!tvControlsVisible) {
                        if (event.type == KeyEventType.KeyDown) {
                            if (event.key == androidx.compose.ui.input.key.Key.DirectionLeft) currentQueueTvSeek(-TV_SEEK_STEP_MS)
                            else currentQueueTvSeek(TV_SEEK_STEP_MS)
                            seekText = if (event.key == androidx.compose.ui.input.key.Key.DirectionLeft) "-30 sec" else "+30 sec"
                            seekTextVisible = true
                        }
    ...
    Box(modifier.then(remoteModifier)) {
        ...
        if (seekTextVisible) {
            Box(
                modifier = Modifier.align(Alignment.Center).background(Color.Black.copy(alpha = 0.5f), RoundedCornerShape(8.dp)).padding(horizontal = 16.dp, vertical = 8.dp)
            ) {
                Text(seekText, color = Color.White, style = MaterialTheme.typography.titleLarge)
            }
        }
        ...
        LaunchedEffect(action?.first, tvControlsVisible) {
            if (device.isTv && action != null && !tvControlsVisible) {
                delay(32)
        ...
        action?.let { (label, onClick) ->
            ...
            WebSkipButton(
                ...
                }).focusRequester(actionFocusRequester).onFocusChanged { isActionFocused = it.isFocused },
            )
        }
    }
}
...
@Composable
private fun WebSkipButton(label: String, onClick: () -> Unit, isFocused: Boolean = false, modifier: Modifier = Modifier) {
    OutlinedButton(
        onClick = onClick,
