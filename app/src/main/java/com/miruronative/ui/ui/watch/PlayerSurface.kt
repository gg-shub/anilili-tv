...
fun PlayerSurface(
        ...
        Modifier
            ...
            .onPreviewKeyEvent { event ->
                ...
                if (!tvControlsVisible) {
                    DiagnosticsLog.event("PlayerSurface TV controls key=${event.key}")
                    when (event.key) {
                        androidx.compose.ui.input.key.Key.DirectionLeft -> {
                            queueTvSeek(-TV_SEEK_STEP_MS)
                            seekIndicatorText = "-30 sec"
                        }
    ...
    Box(modifier.then(remoteModifier)) {
        ...
        LaunchedEffect(action?.first, controlsVisible) {
            if (device.isTv && action != null && !controlsVisible) {
                delay(32)
        ...
        action?.let { (label, onClick) ->
            val actionModifier = Modifier
                ...
                .padding(start = 32.dp, bottom = if (controlsVisible) 96.dp else 32.dp)
                .focusRequester(actionFocusRequester)
                .onFocusChanged { isActionFocused = it.isFocused }
                .onPreviewKeyEvent { event ->
                    if (event.type == KeyEventType.KeyDown &&
                        (event.key == androidx.compose.ui.input.key.Key.DirectionUp || event.key == androidx.compose.ui.input.key.Key.DirectionDown)
                    ) {
                        if (!controlsVisible && device.isTv) {
                            tvControlsVisible = true
                            tvControlsInteraction++
                            return@onPreviewKeyEvent true
                        }
            ...
            OutlinedButton(
                ...
                colors = ButtonDefaults.outlinedButtonColors(
                    containerColor = if (isActionFocused) Color.White else Color.Black.copy(alpha = 0.5f),
                    contentColor = if (isActionFocused) Color.Black else Color.White,
                ),
