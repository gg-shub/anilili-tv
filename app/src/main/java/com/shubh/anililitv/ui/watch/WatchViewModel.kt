...
import com.shubh.anililitv.ui.detail.mergeEpisodeMetadata
...
class WatchViewModel : ViewModel() {
    ...
    private fun launchMiruroLateMerge(id: Int, deferred: kotlinx.coroutines.Deferred<Result<EpisodesResult>>) {
        ...
        miruroLateMergeJob = viewModelScope.launch {
            ...
            val number = lastRequestedNumber
            spine = ensureSpineContains(spine, number)
    ...
    private fun ensureSpineContains(currentSpine: List<EpisodeItem>, number: Double): List<EpisodeItem> {
        if (currentSpine.any { it.number == number }) return currentSpine
        val missingEp = mergedEpisodes.providers
            .flatMap { it.episodes(category) }
            .firstOrNull { it.number == number } ?: return currentSpine
        val merged = (currentSpine + missingEp).distinctBy { it.number }.sortedBy { it.number }
        if (episodeMeta.isEmpty()) return merged
        return mergeEpisodeMetadata(merged, episodeMeta, anilistId)
    }
    ...
    private suspend fun loadEpisodeMetadata(id: Int) {
        ...
        val aniZipEpisodes = withTimeoutOrNull(3000) {
            kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
                runCatching {
                    val url = "https://api.ani.zip/mappings?anilist_id=$id"
        ...
        val mergedKonohaMap = mutableMapOf<Double, KonohaEpisode>()
        meta.forEach { ep -> ep.number?.let { mergedKonohaMap[it] = ep } }
        aniZipList.forEach { ep -> ep.number?.let { mergedKonohaMap[it] = ep } }
        val konohaEpisodes = mergedKonohaMap.values.sortedBy { it.number ?: 0.0 }
        
        if (konohaEpisodes.isEmpty() || id != anilistId) return
        episodeMeta = konohaEpisodes
        val data = (_state.value as? UiState.Success)?.data ?: return
        if (data.episodes.isEmpty()) return
        _state.value = UiState.Success(
            data.copy(episodes = mergeEpisodeMetadata(data.episodes, konohaEpisodes, id)),
        )
    }
    ...
    private fun switchSource(providerName: String, categoryApi: String, rememberProvider: Boolean) {
        ...
        val currentNumber = lastRequestedNumber
    ...
    fun onPlaybackError(message: String, streamUrl: String, positionMs: Long) {
        ...
        val number = lastRequestedNumber
        unavailableSources += EpisodeSourceKey(number, data.provider, data.category)
        launchResolve(number) {
            _state.value = UiState.Success(
                data.copy(
                    isResolving = true,
