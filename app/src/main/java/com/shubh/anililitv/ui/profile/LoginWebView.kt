...
private const val TV_FOCUS_OAUTH_ALLOW_JS = """
    (function() {
        var allowBtn = Array.prototype.slice.call(document.querySelectorAll('button, input[type="submit"], input[type="button"], a.btn, a.button')).filter(function(b) {
            var text = (b.value || b.textContent || '').trim().toLowerCase();
            return text === 'allow' || text === 'authorize' || text === 'yes';
        })[0];
        if (allowBtn) {
            allowBtn.focus();
            try { allowBtn.scrollIntoView({block: 'center'}); } catch(e) {}
        } else if (window.__miruroTvFocusInitial) {
            window.__miruroTvFocusInitial();
        }
    })();
"""
                                        ...
                                        view?.evaluateJavascript(TV_EDITOR_JS) {
                                            view.postDelayed({
                                                if (view.isAttachedToWindow) {
                                                    view.evaluateJavascript(TV_FOCUS_OAUTH_ALLOW_JS, null)
                                                }
                                            }, 180)
                                        }
                                    }
...
private const val LOGIN_LOAD_TIMEOUT_MS = 30_000L
