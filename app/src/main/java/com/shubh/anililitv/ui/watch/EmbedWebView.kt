    ...
    val webClient = remember(allowedHost, navigationLock) {
        object : WebViewClient() {
            override fun shouldOverrideUrlLoading(view: WebView?, request: WebResourceRequest?): Boolean {
                ...
                if (navigationLock.locked) return true
                ...
                if (!tvControlsVisible) {
                    when (event.key) {
                        ...
                        androidx.compose.ui.input.key.Key.NumPadEnter -> {
                            ...
                            if (opensTvPlayerControls(event.key)) {
                                tvControlsVisible = true
                                return@onPreviewKeyEvent true
                            }
                        }
    ...
    Box(modifier.then(remoteModifier)) {
        ...
        AndroidView(
            ...
            factory = { ctx ->
                try {
                    ...
                    object : WebView(ctx) {
                            ...
                            if (event.action == KeyEvent.ACTION_DOWN && event.repeatCount == 0) {
                                when {
                                    device.isTv && opensTvPlayerControls(event.keyCode) -> {
                                        mainHandler.post {
                                            if (!tvControlsVisible && !skipActionFocused) {
                                                tvControlsVisible = true
                                            }
                                            tvControlsInteraction++
                                        }
                        ...
                        isFocusable = !device.isTv || (focusPlayerOnStart && !tvControlsVisible)
                        isFocusableInTouchMode = !device.isTv || (focusPlayerOnStart && !tvControlsVisible)
                        with(settings) {
            ...
            update = { view ->
                ...
                web.isFocusable = !device.isTv || (focusPlayerOnStart && !tvControlsVisible)
                web.isFocusableInTouchMode = !device.isTv || (focusPlayerOnStart && !tvControlsVisible)
            ...
            if (onToggleFullscreen != null) {
                androidx.compose.material3.IconButton(
                    ...
                    modifier = Modifier
                        ...
                        .statusBarsPadding()
                        .padding(4.dp)
                        .focusHighlight(androidx.compose.foundation.shape.RoundedCornerShape(24.dp)),
                ) {
        ...
        val skipActionFocus = remember { androidx.compose.ui.focus.FocusRequester() }
        ...
        action?.let { (label, onClick) ->
            ...
            WebSkipButton(
                ...
                }.focusRequester(skipActionFocus).onFocusChanged { skipActionFocused = it.isFocused },
            )
        }
    }
}
