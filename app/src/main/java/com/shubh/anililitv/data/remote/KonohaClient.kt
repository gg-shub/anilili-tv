...
class KonohaClient(
    ...
    private val aniZipClient = AniZipClient(client, json)
    ...
    suspend fun getMetadata(anilistId: Int): AniZipClient.AniZipMetadata? {
        return aniZipClient.getMetadata(anilistId)
    }
    ...
    suspend fun episodes(anilistId: Int): List<KonohaEpisode> = withContext(Dispatchers.IO) {
        val mappedAnilistId = aniZipClient.resolveMapping(anilistId) ?: anilistId
        cache.getOrFetch(
            key = "konohaeps:v1:$mappedAnilistId",
            serializer = ListSerializer(KonohaEpisode.serializer()),
            ttlMs = EPISODES_TTL_MS,
        ) {
            val shard = mappedAnilistId / 1000
            val url = "https://cdn.jsdelivr.net/gh/AlokRepo/Konoha@main/data/anime/$shard/$mappedAnilistId/episodes.json"
            val request = Request.Builder()
                .url(url)
                .header("Accept", "application/json")
                .build()
    ...
    private companion object {
        const val TTL_HIT_MS = 30L * 24 * 60 * 60 * 1000
